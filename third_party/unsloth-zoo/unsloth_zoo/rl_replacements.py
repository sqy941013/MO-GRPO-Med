...
def grpo_compute_loss(
    ...
    **kwargs
    ...
    # Huberized advantage clipping threshold (default 0.65)
    advantage_delta = kwargs.get("advantage_delta", 0.65)
    # Toggle for applying advantage_delta clipping (default OFF for safety)
    use_advantage_delta = kwargs.get("use_advantage_delta", False)
    ...
    # 记录PPO clipping统计 (避免在编译函数中使用print)
    try:
        # 计算PPO clipping统计
        ratio_flat = coef_1.flatten()
        lower_bound = 1.0 - epsilon_low
        upper_bound = 1.0 + epsilon_high
        
        clipped_mask = (ratio_flat < lower_bound) | (ratio_flat > upper_bound)
        clipped_count = torch.sum(clipped_mask).item()
        total_count = ratio_flat.numel()
        
        # 存储到全局变量供外部访问
        if hasattr(torch, '_ppo_stats'):
            torch._ppo_stats = {
    ...
    # Huberize/clip the sample-level advantages to stabilize updates (optional)
    if use_advantage_delta:
        clipped_advantages = torch.clamp(advantages, -advantage_delta, advantage_delta)
        
        # 记录Huber截断统计 (避免在编译函数中使用print)
        try:
            # 使用torch操作计算统计，避免print
            orig_flat = advantages.flatten()
            clip_flat = clipped_advantages.flatten()
            clipped_mask = torch.abs(orig_flat - clip_flat) > 1e-6
            clipped_count = torch.sum(clipped_mask).item()
            total_count = orig_flat.numel()
            
            # 存储到全局变量供外部访问 (避免import和print)
            if hasattr(torch, '_huber_stats'):
                torch._huber_stats = {
                    'clipped_ratio': clipped_count / total_count if total_count > 0 else 0.0,
                    'clipped_count': clipped_count,
                    'total_count': total_count,
        ...
        clipped_advantages = advantages
    if delta is not None:
        loss_1 = torch.clamp(coef_1, max=delta) * clipped_advantages.unsqueeze(1)
    else:
        loss_1 = coef_1 * clipped_advantages.unsqueeze(1)
    pass
    ...
    loss_2 = coef_2 * clipped_advantages.unsqueeze(1)
...
def grpo_accumulated_loss(
    ...
    **kwargs,
    ...
    with torch.amp.autocast(device_type = trainer.model.device.type, dtype = trainer._autocast_dtype):
        ...
        # Merge CLI-propagated args from trainer.args into extra kwargs
        try:
            if hasattr(trainer, "args"):
                args = trainer.args
                # Only set when provided on args; keep explicit kwargs precedence
                if "use_advantage_delta" not in kwargs and hasattr(args, "use_advantage_delta"):
                    kwargs["use_advantage_delta"] = bool(getattr(args, "use_advantage_delta"))
                if "advantage_delta" not in kwargs and hasattr(args, "advantage_delta") and getattr(args, "advantage_delta") is not None:
                    kwargs["advantage_delta"] = float(getattr(args, "advantage_delta"))
        except Exception:
            pass
